...
if (subprocess.run(["javac","-encoding","utf8","-cp",(";" if os.name=="nt" else ":").join(ml),"-d","build"]+jfl).returncode!=0):
	sys.exit(1)
with zipfile.ZipFile("build/fischertechnic_dissasembler.jar","w") as zf:
...
if ("--run" in sys.argv):
	subprocess.run(["java","-Dfile.encoding=UTF8","-jar","build/fischertechnic_dissasembler.jar","temp.rpp"])
